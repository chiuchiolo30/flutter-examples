...
class ShapesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 60; i++) {
      ...
      final x1 = (size.width / 2) + (size.width / 3 + distance) * cos(Vector.radians(minute));
      final y1 = (size.height / 2) + (size.width / 3 + distance) * sin(Vector.radians(minute));
      final p1 = Offset(x1, y1);

      final x2 = (size.width / 2) + (size.width / 3 + 30) * cos(Vector.radians(minute));
      final y2 = (size.height / 2) + (size.width / 3 + 30) * sin(Vector.radians(minute));
      final p2 = Offset(x2, y2);

      // canvas.drawLine(p1, p2, paint);
      // pinta los números de 00 a 59
      ...
      tp.layout();
      // tp.paint(canvas, Offset(x1 - 10, y1 - 10));
      canvas.save();
      canvas.translate(x1, y1);
      canvas.rotate(Vector.radians(90));
